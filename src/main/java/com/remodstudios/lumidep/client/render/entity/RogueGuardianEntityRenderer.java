...
import com.remodstudios.lumidep.client.render.entity.model.RogueGuardianEntityModel;
...
// TODO
// MobEntityRenderer<GuardianEntity, RogueGuardianEntityModel> -> MobEntityRenderer<RogueGuardianEntity, RogueGuardianEntityModel>
public class RogueGuardianEntityRenderer extends MobEntityRenderer<GuardianEntity, RogueGuardianEntityModel> {
    private static final Identifier TEXTURE = LuminousDepths.id("textures/entity/rogue_guardian.png");
    private static final Identifier EXPLOSION_BEAM_TEXTURE = new Identifier("textures/entity/guardian_beam.png");
    private static final RenderLayer LAYER;

    public RogueGuardianEntityRenderer(EntityRendererFactory.Context context) {
        this(context, 0.5F, EntityModelLayers.GUARDIAN);
    }

    protected RogueGuardianEntityRenderer(EntityRendererFactory.Context ctx, float shadowRadius, EntityModelLayer layer) {
        super(ctx, new RogueGuardianEntityModel(ctx.getPart(layer)), shadowRadius);
    }
    ...
    public void render(GuardianEntity guardianEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        ...
        if (livingEntity != null) {
            ...
            float v = 0.2F;
            float w = 0.282F;
            float x = MathHelper.cos(q + 2.3561945F) * w;
            float y = MathHelper.sin(q + 2.3561945F) * w;
            float z = MathHelper.cos(q + 0.7853982F) * w;
            float aa = MathHelper.sin(q + 0.7853982F) * w;
            float ab = MathHelper.cos(q + 3.926991F) * w;
            float ac = MathHelper.sin(q + 3.926991F) * w;
            float ad = MathHelper.cos(q + 5.4977875F) * w;
            float ae = MathHelper.sin(q + 5.4977875F) * w;
            float af = MathHelper.cos(q + 3.1415927F) * v;
            float ag = MathHelper.sin(q + 3.1415927F) * v;
            float ah = MathHelper.cos(q + 0.0F) * v;
            float ai = MathHelper.sin(q + 0.0F) * v;
            float aj = MathHelper.cos(q + 1.5707964F) * v;
            float ak = MathHelper.sin(q + 1.5707964F) * v;
            float al = MathHelper.cos(q + 4.712389F) * v;
            float am = MathHelper.sin(q + 4.712389F) * v;
            float ao = 0.0F;
            float ap = 0.4999F;
            float aq = -1.0F + k;
            float ar = m * 2.5F + aq;
